...
export async function run(): Promise<void> {
  ...
  await new Promise((resolve) => setTimeout(resolve, 200));
  const invalidDiagnostics = vscode.languages.getDiagnostics(invalid.uri).filter((diagnostic) => diagnostic.source === "htmx-django-intellisense");
  ...
  await new Promise((resolve) => setTimeout(resolve, 200));
  assert.equal(vscode.languages.getDiagnostics(expressionDocument.uri).some((diagnostic) => diagnostic.source === "htmx-django-intellisense"), false);
  ...
  assert.equal(vscode.languages.getDiagnostics(invalid.uri).some((diagnostic) => diagnostic.source === "htmx-django-intellisense"), false);
  await vscode.workspace.getConfiguration("htmxTags").update("enableValidation", true, vscode.ConfigurationTarget.Global);
  await new Promise((resolve) => setTimeout(resolve, 50));
  assert.ok(vscode.languages.getDiagnostics(invalid.uri).some((diagnostic) => diagnostic.source === "htmx-django-intellisense"));
