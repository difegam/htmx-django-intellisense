...
- Stable HTMX 4 catalog, including `hx-query`, `hx-action`, and morph-skip controls.

- HTMX 4 config and status value completions, `outerSync`, scroll targets, `focusScroll`, and event modifiers.

- HTMX 4 validation-response, morph-refresh, and multi-target response snippets.

- Offline HTMX catalog covering both HTMX `2.0.10` and `4.0.0`, with `compatible` default mode accepting their union.
...
- HTMX 4 inline event snippets use colon-separated lifecycle names; HTMX 2 retains its original syntax.

- HTMX 4 modifiers are excluded from HTMX 2 completions and correctly produce version hints.

- Editing snippets put target and swap attributes on request controls so they work without implicit inheritance.

- Catalog generation supports the stable release documentation layout; hover explains the changed meaning of `hx-disable`.
...
- HTMX 4 pin bumped from `4.0.0-beta6` to `4.0.0`; catalog regenerated.
